...
void initializeBluetooth(char *MACAddr) {	
	...
	if (error) {
        ...
        [LVControllerInstance setStatus:@"BTdaemon failed." withExit:TRUE];
		return;
	}
	bt_register_packet_handler(packet_handler);
	bt_send_cmd(&btstack_set_power_mode, HCI_POWER_ON);
	[LVControllerInstance setStatus:@"Initializing..." withExit:FALSE];
	run_loop_execute();
	bt_close();
}
		...
		case HCI_EVENT_PACKET:
			switch (packet[0]) {
				case BTSTACK_EVENT_POWERON_FAILED:
					...
					[LVControllerInstance setStatus:@"Initialization failed." withExit:TRUE];
					[LVControllerInstance setInitialized:FALSE];
					break;		
					
				case BTSTACK_EVENT_STATE:
                    if (packet[2] == HCI_STATE_WORKING) {
                        printf("BTstack activated.\n");
                        [LVControllerInstance setInitialized:TRUE];
                        [LVControllerInstance setStatus:@"Connecting..." withExit:FALSE];
				...
				case RFCOMM_EVENT_OPEN_CHANNEL_COMPLETE:
					...
					if (packet[2]) {
                        ...
                        [LVControllerInstance setStatus:@"Connection failed." withExit:TRUE];
					} else {
                        ...
                        [LVControllerInstance sendGetCaps];
                        [LVControllerInstance setConnected:TRUE];
                        [LVControllerInstance setStatus:@"Connected." withExit:FALSE];
					}
					break;
                    
				case HCI_EVENT_DISCONNECTION_COMPLETE:
					printf("Basebank connection closed\n");
                    [LVControllerInstance setStatus:@"Connection closed." withExit:TRUE];
					[LVControllerInstance setConnected:FALSE];
					break;
